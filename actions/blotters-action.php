<?php if ( ! defined('ACCESS') ) die("Direct access not allowed.");
	...
	if ( isset($_POST['update-blotters']) ) {
        ...
        $update_blotters = $DB->prepare("UPDATE blotter SET complainant = ?, c_address = ?, c_contact = ?, person_to_complain = ?, p_address = ?, p_contact = ?, action_taken = ?, complaint_status = ?, location_of_incidence = ?, complaint = ? WHERE blotterID = ?");

        try {
            $DB->beginTransaction();
            if( $update_blotters->execute( [ $complainant, $c_address, $c_contact, $person_to_complain, $p_address, $p_contact, $action_taken, $complaint_status, $location_of_incidence, $complaint, $blotterID] ) ) {
                $DB->commit();
